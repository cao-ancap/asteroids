...
func make_post_request(url: String, data_to_send: Dictionary, use_ssl := true):
	...
	http_request.connect("request_completed", self, "_on_post_request_completed")
	var error := http_request.request(url, headers, use_ssl, HTTPClient.METHOD_POST, request_data)
	if error != OK:
...
func make_get_request(url: String):
	var http_request := HTTPRequest.new()
	add_child(http_request)
	http_request.connect("request_completed", self, "_on_get_request_completed")
